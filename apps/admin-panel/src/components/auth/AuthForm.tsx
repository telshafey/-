...
export const AuthForm: React.FC<AuthFormProps> = ({
    ...
    if (userResult) {
      ...
      if (allowedRoles && allowedRoles.length > 0) {
        if (!allowedRoles.includes(userResult.role)) {
          ...
          setLocalError(
            "غير مسموح لهذا النوع من الحسابات بالدخول من هذه البوابة.",
          );
          return;
        }
      }
      ...
      if (redirectTo) {
        ...
        if (userResult.role === "student") {
          ...
          ["super_admin", "instructor", "general_supervisor"].includes(
            userResult.role,
          )
        ) {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLocalError(
        err.message || "فشل إرسال الرابط. تأكد من صحة البريد الإلكتروني.",
      );
    } finally {
  ...
  return (
    <div className="w-full bg-white p-8 rounded-2xl shadow-lg border">
      ...
      >
        ...
        {localError && (
          <div
            className={`p-3 rounded border flex items-start gap-2 text-sm font-semibold ${redirecting ? "bg-blue-50 text-blue-800 border-blue-200" : "bg-red-50 text-red-800 border-red-200"}`}
          >
            {redirecting ? (
